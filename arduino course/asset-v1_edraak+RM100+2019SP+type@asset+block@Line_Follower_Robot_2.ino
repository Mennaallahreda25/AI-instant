...
#define sensor2   A2
#define sensor3   A1
#define sensor4   A0      // Right most sensor
...
int sensor[4] = {0, 0, 0, 0}; // Initial Values of Sensors
...
void loop() {
  ...
  Read_MotorSpeed_POT(); //Read Speed Potintiometer and set motors speeds


}

////////////////////////////////////
//Read_MotorSpeed_POT Function
//Read Speed Potintiometer and set motors speeds
////////////////////////////////////
void Read_MotorSpeed_POT() {

  MotorSpeed = analogRead(MotorSpeed_POT);

  Serial.print("MotorSpeed =   ");
  Serial.println(map(MotorSpeed, 0, 1023, 40 , 100));

  analogWrite(ENA, (map(MotorSpeed, 0, 1023, 40 , 100))); // Send PWM signal to L298N Enable pin
  analogWrite(ENB, (map(MotorSpeed, 0, 1023, 40 , 100))); // Send PWM signal to L298N Enable pin
...
void Read_sensor_values()
{
  ...
  if ((sensor[0] == 0) && (sensor[1] == 1) && (sensor[2] == 1) && (sensor[3] == 0)) { // Move Robot forward
  ...
  else if ((sensor[0] == 1) && (sensor[1] == 1) && (sensor[2] == 0) && (sensor[3] == 0)) {// Turn robot right side
  ...
  else if ((sensor[0] == 0) && (sensor[1] == 0) && (sensor[2] == 1) && (sensor[3] == 1)) { // Turn robot left side
    \
    if (LastState != 3) {
      LastState = 3 ;
    }
  ...
  else if ((sensor[0] == 0) && (sensor[1] == 0) && (sensor[2] == 0) && (sensor[3] == 0)) { // Out of track
  ...
  else if ((sensor[0] == 1) && (sensor[1] == 1) && (sensor[2] == 1) && (sensor[3] == 1)) { // Stop Robot
